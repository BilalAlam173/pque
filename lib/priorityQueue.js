...
export class PriorityQueue {
    ...
    #heapify(i) {
        ...
        const right = (2 * i) + 2;
        let min = i;
        if (left <= this.#HEAP_TREE.length && this.#HEAP_TREE[left] < this.#HEAP_TREE[i]) {
            min = left;
        }
        if (right <= this.#HEAP_TREE.length && this.#HEAP_TREE[right] < this.#HEAP_TREE[i]) {
            min = right;
        }
        if (min != i) {
            this.#swap(i, min);
    ...
    constructor(input) {
        ...
        if (input && input.length > 0) {
            const pivot = Math.ceil(input.length / 2) - 1;
